...
void update_bytes_seen(const uint8_t p_block[AES_BLOCK_SIZE], uint8_t bytes_seen[16][256]) {
    for (size_t i = 0; i < AES_BLOCK_SIZE; i++) {
        bytes_seen[i][p_block[i]]++;
    }
}
 ...
 */
void aes128_encrypt(uint8_t p_block[AES_BLOCK_SIZE], const uint8_t p_key_schedule[AES128_KEY_SCHEDULE_SIZE], uint8_t bytes_seen[16][256])
{
    uint_fast8_t    round;

    aes_block_xor(p_block, p_key_schedule);
    update_bytes_seen(p_block, bytes_seen);
